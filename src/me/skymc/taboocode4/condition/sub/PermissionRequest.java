...
public class PermissionRequest implements NormalCondition {
	...
	public boolean register(FileConfiguration conf, boolean first) {
		file = conf;
		if (first) {
			conf.set("Settings.Pattern", "[$] Permission Request");
		...
		pattern = Pattern.compile(conf.getString("Settings.Pattern").replace("[#]", "(?:,|])").replace("[$]", "(\\S+)").replace("[?]", "(?:\\s|\\S)"));
		return true;
	}
	...
	public boolean execute(Player player, ItemStack item, String lore, List<String> uncoloredlore) {
		...
		if (m.find()) {
			if (!player.hasPermission(m.group(group_$))) {
				return false;
			}
		}
		return true;
