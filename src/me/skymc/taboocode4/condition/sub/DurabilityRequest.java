...
public class DurabilityRequest implements NormalCondition {
	...
	public boolean register(FileConfiguration conf, boolean first) {
		file = conf;
		if (first) {
			conf.set("Settings.Pattern", "[$] Durability Limit");
	...
	public boolean execute(Player player, ItemStack item, String lore, List<String> uncoloredlore) {
		...
		if (m.find()) {
			if (item.getDurability() > Double.valueOf(m.group(group_$))) {
				return false;
			}
		}
		return true;
