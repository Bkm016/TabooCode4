...
public class SkillAPISkillLevelRequest implements NormalCondition {
	...
	public boolean register(FileConfiguration conf, boolean first) {
		file = conf;
		if (first) {
			conf.set("Settings.Pattern", "[$][?]SkillLevel[?][S][?]Request");
			
			conf.set("Settings.GroupLocation.Value", 1);
			conf.set("Settings.GroupLocation.SkillName", 2);
		...
		group_P = conf.getInt("Settings.GroupLocation.SkillName");
		
		pattern = Pattern.compile(conf.getString("Settings.Pattern").replace("[$]", "((?:\\-|\\+)?\\d+(?:\\.\\d+)?)").replace("[?]", "(?:\\s|\\S)").replace("[S]", "(\\S+)"));
	...
	public boolean execute(Player player, ItemStack item, String unclore, List<String> uncoloredlore) {
		...
		for (String lore : uncoloredlore) {
			...
			if (m.find()) {
				...
				if (data.getSkillLevel(m.group(group_P)) < Double.valueOf(m.group(group_$))) {
					return false;
				}
			}
		}
