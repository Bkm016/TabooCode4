...
public class SkillAPIMaxManaRequest implements NormalCondition {
	...
	public boolean register(FileConfiguration conf, boolean first) {
		file = conf;
		if (first) {
			conf.set("Settings.Pattern", "[$][?]MaxMana Request");
	...
	public boolean execute(Player player, ItemStack item, String lore, List<String> uncoloredlore) {
		...
		if (m.find()) {
			...
			if (data.getMaxMana() < Double.valueOf(m.group(group_$))) {
				return false;
			}
		}
