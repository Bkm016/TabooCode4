...
public class SkillAPIClassLevelRequest implements NormalCondition {
	...
	public boolean register(FileConfiguration conf, boolean first) {
		file = conf;
		if (first) {
			conf.set("Settings.Pattern", "[$][?]ClassLevel[?][C][?]Request");
			
			conf.set("Settings.GroupLocation.Value", 1);
			conf.set("Settings.GroupLocation.ClassName", 2);
		...
		group_P = conf.getInt("Settings.GroupLocation.ClassName");
		
		pattern = Pattern.compile(conf.getString("Settings.Pattern").replace("[$]", "((?:\\-|\\+)?\\d+(?:\\.\\d+)?)").replace("[?]", "(?:\\s|\\S)").replace("[C]", "(\\S+)"));
	...
	public boolean execute(Player player, ItemStack item, String _lore, List<String> uncoloredlore) {
		...
		for (String lore : uncoloredlore) {
			...
			if (m.find()) {
				...
				if (data.getClass(m.group(group_P)).getLevel() < Double.valueOf(m.group(group_$))) {
					return false;
				}
			}
		}
