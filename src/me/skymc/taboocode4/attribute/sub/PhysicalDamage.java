...
public class PhysicalDamage implements SpecialAttribute {
	...
	public boolean register(FileConfiguration conf, boolean first) {
		file = conf;
		if (first) {
			conf.set("Settings.Primary", "Physical Damage: [$][?][$]");
			conf.set("Settings.Regular", "[$]\\* Physical Damage");
			conf.set("Settings.Percent", "[$]\\% Physical Damage");
	...
	public double execute(LivingEntity attacker, LivingEntity victim, HashMap<String, SpecialAttributeData> attackerattr, HashMap<String, SpecialAttributeData> victimattr, Event event) {
		...
		if (victimattr != null) {
			Double defense = AttributesManager.getPrimaryValue(victim, "PhysicalDefense") + AttributesManager.getRegularValue(victim, "PhysicalDefense");
			Double defense2 = defense + (defense * AttributesManager.getPercentValue(victim, "PhysicalDefense"));
