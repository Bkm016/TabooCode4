...
public class PlayerDamage implements SpecialAttribute {
	...
	public boolean register(FileConfiguration conf, boolean first) {
		file = conf;
		if (first) {
			conf.set("Settings.Primary", "Player Damage: [$][?][$]");
			conf.set("Settings.Regular", "[$]\\* Player Damage");
			conf.set("Settings.Percent", "[$]\\% Player Damage");
	...
	public double execute(LivingEntity attacker, LivingEntity victim, HashMap<String, SpecialAttributeData> attackerattr, HashMap<String, SpecialAttributeData> victimattr, Event event) {
		...
		if (victim instanceof Player) {
			...
			Double defense = AttributesManager.getPrimaryValue(victim, "PlayerDefense") + AttributesManager.getRegularValue(victim, "PlayerDefense");
			Double defense2 = defense + (defense * AttributesManager.getPercentValue(victim, "PlayerDefense"));
