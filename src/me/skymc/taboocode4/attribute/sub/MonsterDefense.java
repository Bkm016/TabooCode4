...
public class MonsterDefense implements SpecialAttribute {
	...
	public boolean register(FileConfiguration conf, boolean first) {
		file = conf;
		if (first) {
			conf.set("Settings.Primary", "Monster Defense: [$][?][$]");
			conf.set("Settings.Regular", "[$][*][ ]Monster Defense");
			conf.set("Settings.Percent", "[$][%][ ]Monster Defense");
	...
	public double execute(LivingEntity attacker, LivingEntity victim, HashMap<String, SpecialAttributeData> attackerattr, HashMap<String, SpecialAttributeData> victimattr, Event event) {
		...
		if (attacker instanceof Monster && !(attacker instanceof Player)) {
			...
			Double value = AttributesManager.getPrimaryValue(attacker, Name) + AttributesManager.getRegularValue(attacker, Name);
			Double value2 = value + (value * AttributesManager.getPercentValue(attacker, Name));
			
			e.setDamage(e.getDamage() - value2);
		}
		return 0;
	}
}