...
public class CritDamage implements SpecialAttribute {
	...
	public boolean register(FileConfiguration conf, boolean first) {
		file = conf;
		if (first) {
			conf.set("Settings.Primary", "Crit Damage: [$][?][$]");
			conf.set("Settings.Regular", "[$]\\* Crit Damage");
			conf.set("Settings.Percent", "[$]\\% Crit Damage");
