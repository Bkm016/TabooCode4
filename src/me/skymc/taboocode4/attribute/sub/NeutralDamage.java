...
public class NeutralDamage implements SpecialAttribute {
	...
	public boolean register(FileConfiguration conf, boolean first) {
		file = conf;
		if (first) {
			conf.set("Settings.Primary", "Neutral Damage: [$][?][$]");
			conf.set("Settings.Regular", "[$]\\* Neutral Damage");
			conf.set("Settings.Percent", "[$]\\% Neutral Damage");
	...
	public double execute(LivingEntity attacker, LivingEntity victim, HashMap<String, SpecialAttributeData> attackerattr, HashMap<String, SpecialAttributeData> victimattr, Event event) {
		...
		if (victimattr != null) {
			Double defense = AttributesManager.getPrimaryValue(victim, "NeutralDefense") + AttributesManager.getRegularValue(victim, "NeutralDefense");
			Double defense2 = defense + (defense * AttributesManager.getPercentValue(victim, "NeutralDefense"));
