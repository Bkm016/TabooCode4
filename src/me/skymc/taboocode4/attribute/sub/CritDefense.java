...
public class CritDefense implements SpecialAttribute {
	...
	public boolean register(FileConfiguration conf, boolean first) {
		file = conf;
		if (first) {
			conf.set("Settings.Primary", "Crit Defense: [$][?][$]");
			conf.set("Settings.Regular", "[$]\\* Crit Defense");
			conf.set("Settings.Percent", "[$]\\% Crit Defense");
